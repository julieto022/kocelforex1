            ...
            {connections.map((connection) => (
              <li key={connection.id} className="space-y-3 px-4 py-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0"><p className="text-sm font-semibold text-foreground">{connection.account_name}</p><p className="num text-xs text-muted-foreground">{connection.broker?.name ?? "MT5 broker"} · {maskLogin(connection.mt5_login)} · {connection.server}</p></div>
                  <div className="flex items-center gap-2"><StatusBadge tone={connection.environment === "real" ? "info" : "neutral"} size="sm">{connection.environment}</StatusBadge><BridgeStatusBadge status={connection.status} /></div>
                </div>
