      ...
      >
        {isLoading ? (
          ...
          <EmptyState
            ...
            description="Open MetaTrader 5, run the Kocel Bridge EA, and approve its browser authorization request."
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="num text-xs text-muted-foreground">
                      {connection.broker_name ?? connection.broker?.name ?? "MT5 broker"} ·{" "}
                      {maskLogin(connection.mt5_login)} · {connection.server}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <StatusBadge
                      tone={connection.environment.toUpperCase() === "REAL" ? "info" : "neutral"}
                      size="sm"
                    >
