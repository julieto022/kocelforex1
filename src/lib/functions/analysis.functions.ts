...
export const analyzeBot = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<TradingSignal> => {
    ...
    const base = {
      id: crypto.randomUUID(), botId: bot.id, strategyId: bot.strategy_id, connectionId: bot.broker_connection_id ?? "",
      ...
      dataStatus: "UNAVAILABLE" as const, strategyVersion: "unknown",
    } satisfies Omit<TradingSignal, "strategyVersion"> & { strategyVersion: string };

    if (!bot.enabled || bot.status !== "RUNNING") return { ...base, state: "WAIT", reason: "BOT_INACTIVE" };
    if (!bot.broker_connection_id) return { ...base, reason: "CONNECTION_NOT_FOUND" };
    if (!bot.timeframe) return { ...base, reason: "INVALID_TIMEFRAME" };
    ...
    if (!connection) return { ...base, connectionId: bot.broker_connection_id, reason: "CONNECTION_NOT_FOUND" };
    const lastSeen = connection.last_seen_at ? Date.parse(connection.last_seen_at) : NaN;
    if (connection.status !== "CONNECTED") return { ...base, connectionId: connection.id, reason: `CONNECTION_${connection.status}` };
    if (!Number.isFinite(lastSeen) || Date.now() - lastSeen > FRESHNESS_MS) return { ...base, connectionId: connection.id, dataStatus: "STALE", reason: "STALE_DATA" };

    const db = supabase as unknown as { from: (table: string) => any };
    const { data: strategy } = await supabase.from("strategies").select("id, slug, is_active").eq("id", bot.strategy_id).maybeSingle();
    ...
    const { data: rows, error: candleError } = await db.from("market_candles").select("timestamp, open, high, low, close, volume").eq("broker_connection_id", connection.id).eq("symbol", bot.symbol).eq("timeframe", bot.timeframe).order("timestamp", { ascending: true }).limit(250);
    if (candleError || !rows?.length) return { ...base, connectionId: connection.id, strategyId: strategy.id, strategyVersion: definition.version, reason: "MARKET_DATA_UNAVAILABLE" };
    const market = normalizeMarketData(bot.symbol, bot.timeframe, rows as MarketCandle[], 50);
    ...
    const result = definition.analyze({ botId: bot.id, strategyId: strategy.id, symbol: bot.symbol, timeframe: bot.timeframe, configuration: (bot.configuration ?? {}) as Record<string, unknown>, market, candles: market.candles, indicators: calculateIndicators(market.candles) });
    const signal: TradingSignal = { ...base, connectionId: connection.id, strategyId: strategy.id, strategyVersion: definition.version, ...result, dataTimestamp: market.latestTimestamp, dataStatus: "FRESH" };
